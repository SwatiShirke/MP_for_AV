...
def generate_sine_wave_track(num_points=200, length=200, amplitude=10, frequency=0.05):
    x = np.linspace(0, length, num_points)
...
def generate_figure_eight_track(num_points=100, length=2 * np.pi, amplitude=20):
    t = np.linspace(0, length, num_points)
    x = amplitude * np.sin(t)  # Horizontal sinusoidal motion
    y = amplitude * np.sin(2 * t)  # Double frequency for vertical motion
    return x, y
...
if __name__ == '__main__':
    ...
    main(track_type='line',num_points=300, length=100)
