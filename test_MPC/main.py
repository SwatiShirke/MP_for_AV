...
import ipdb
from utils import get_loc_list, warm_start, print_cbf_constraints 
...
x_obj = np.array([Xref[30], Yref[30] + 5, Yawref[30], Sref[30]])
vel = 5 
h_x0_list = [] 
#simulate
for i in range(Nsim):
    ...
    if hx_0[0] > 1:
        #hx_0[0] = 0
        h_lambda_bot = np.zeros(4)
        h_lambda_obj = np.zeros(4)
