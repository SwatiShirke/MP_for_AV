...
class BagWriterNode(Node):
    def __init__(self):
        ...
        self.create_bag_topic('predicted_path', 'vd_msgs/msg/VDtraj')
        self.create_bag_topic( '/norm_error', 'std_msgs/msg/Float32')
        self.create_bag_topic('global_path', 'vd_msgs/msg/VDPath')
        self.create_bag_topic('explored_nodes', 'vd_msgs/msg/VDPath') 
        ...
        self.create_subscription(VDpose, "/vehicle_est_pose", self.est_callback, qos_profile)
        ...
        self.create_subscription(VDPath, "explored_nodes", self.explored_nodes_callback, 1)
        self.create_subscription(VDtraj, 'predicted_path', self.mpc_pred_path_callback, 1)
    ...
    def mpc_pred_path_callback(self, msg):
        """ path predicted by MPC given as reference path to PID controller """
        #self.get_logger().info("in mpc pred callback")
        if len(msg.poses) > 0:  # Ensure there is at least one point in the trajectory
            # Get the 0th point (first point)
            first_point = msg.poses[0]
            self.get_logger().info(f"Received MPC predicted path message with first point: {first_point}")
            self.write_to_bag("predicted_path", msg)  # Save the entire TrajCommand message (or just the first point if needed)



    def path_callback(self, msg):
        """save entire path generated by the global planner"""
        if len(msg.x_val) > 0:  # Ensure there is at least one point in the trajectory
            # Get the 0th point (first point)
            
            #self.get_logger().info(f"Received path msg")
            self.write_to_bag('global_path', msg)  # Save the entire TrajCommand message (or just the first point if needed)


    def explored_nodes_callback(self, msg):
        #self.get_logger().info(f"Received explored nodes msg")
        
        """save entire path generated by the global path service."""
        if len(msg.x_val) > 0:  # Ensure there is at least one point in the trajectory
            # Get the 0th point (first point)            
            #self.get_logger().info(f"Received explored nodes msg")
            self.write_to_bag('explored_nodes', msg)  # Save the entire TrajCommand message (or just the first point if needed)
